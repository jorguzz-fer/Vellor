...
export default function OrderDetailPage() {
  ...
  usePageMeta(
    query.data
      ? `Pedido ${query.data.number} · ${t('admin.title')}`
      : `${t('admin.orders')} · ${t('admin.title')}`,
  );
  ...
  const run = (action: OrderAction, extra: Partial<OrderActionInput> = {}) =>
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <section className="card p-4" aria-label="Ações do pedido">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            disabled={mutation.isPending}
            data-testid="order-action-add_note"
          >
            {ORDER_ACTION_LABELS.add_note}
          </Button>
          {available.length === 0 && (
            <span className="text-xs text-muted">
              Pedido {ORDER_STATUS_LABELS[order.status].toLowerCase()}: nenhuma outra transição
              disponível.
            </span>
          )}
        </div>
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <section className="card p-5" aria-label="Cliente">
              <div className="mb-4 flex items-center justify-between gap-2">
                ...
                {order.userId ? (
                  <Link
                    to={`/admin/clientes/${order.userId}`}
                    className="link text-xs uppercase tracking-[0.15em]"
                  >
                    Ver cadastro
                  </Link>
                ) : (
              ...
              <dl className="space-y-3 text-sm">
                ...
                <Row label={t('account.cpf')}>
                  <span className="inline-flex items-center gap-2">
                    ...
                    <CopyButton
                      onClick={() => copy(order.customer.cpf.replace(/\D/g, ''), 'CPF')}
                      label="Copiar CPF"
                    />
                  </span>
                </Row>
            ...
            <section className="card p-5" aria-label="Endereço de entrega">
              ...
              <address className="space-y-0.5 text-sm not-italic text-ivory/90">
                ...
                {address.reference && (
                  <p className="pt-1 text-xs text-muted">Ref.: {address.reference}</p>
                )}
              </address>
            </section>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <section className="card p-5" aria-label="Observações do cliente">
              ...
              {order.notes ? (
                <p className="whitespace-pre-line text-sm text-ivory/90">{order.notes}</p>
              ) : (
            ...
            <section className="card p-5" aria-label="Notas internas">
              ...
              {order.internalNotes ? (
                <p className="whitespace-pre-line font-mono text-xs leading-relaxed text-ivory/90">
                  {order.internalNotes}
                </p>
              ) : (
        ...
        <div className="space-y-6">
          ...
          <section className="card p-5" aria-label="Pagamento">
            ...
            <dl className="space-y-3 text-sm">
              ...
              <Row label="Situação">
                <Badge tone={PAYMENT_TONE[order.payment.status]}>
                  {PAYMENT_STATUS_LABELS[order.payment.status]}
                </Badge>
              </Row>
              <Row label={t('checkout.installments')}>
                {order.payment.installments > 1
                  ? `${order.payment.installments}x de ${formatBRL(order.payment.installmentCents)}`
                  : 'À vista'}
              </Row>
              {order.payment.paidAt && (
                <Row label="Pago em">{formatDateTime(order.payment.paidAt)}</Row>
              )}
              {order.payment.boleto?.dueDate && (
                <Row label="Vencimento do boleto">
                  {formatDateTime(order.payment.boleto.dueDate)}
                </Row>
              )}
              {order.payment.pix?.expiresAt && (
                <Row label="Pix válido até">{formatDateTime(order.payment.pix.expiresAt)}</Row>
              )}
              <Row label="ID no provedor" stack>
                {order.providerPaymentId ? (
                  <span className="inline-flex items-center gap-2">
                    <span className="break-all font-mono text-xs">{order.providerPaymentId}</span>
                    <CopyButton
                      onClick={() => copy(order.providerPaymentId ?? '', 'ID do pagamento')}
              ...
              {order.providerCustomerId && (
                <Row label="Cliente no provedor" stack>
          ...
          <section className="card p-5" aria-label="Envio">
              ...
              <Row label={t('order.tracking')}>
                {order.trackingCode ? (
                  <span className="inline-flex items-center gap-2">
                    ...
                    <CopyButton
                      onClick={() => copy(order.trackingCode ?? '', 'Código de rastreio')}
                      label="Copiar código de rastreio"
                    />
                  </span>
                ) : (
            ...
            {order.trackingUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="btn-secondary mt-4 px-3 py-2 text-[10px]"
              >
      ...
      >
        <form id="order-ship-form" onSubmit={submitShipped} noValidate className="space-y-4">
          <p className="text-sm text-ivory/80">
            O cliente receberá um e-mail com o código de rastreio e o pedido passará para "Enviado".
          </p>
          <Input
      ...
      <ConfirmDialog
        ...
        text={
          <p>
            Use apenas quando o pagamento foi recebido fora do sistema (ex.: transferência conferida
            no extrato). O estoque será baixado e o cliente receberá o e-mail de confirmação do
            pedido <strong className="text-cream">{order.number}</strong>.
          </p>
        }
      />
      ...
      <ConfirmDialog
        ...
        text={
          <div className="space-y-4">
            <p>
              O pedido <strong className="text-cream">{order.number}</strong> será cancelado, o
              estoque liberado e o cliente avisado por e-mail. Cobranças pendentes são canceladas no
      ...
      <ConfirmDialog
        ...
        text={
          <div className="space-y-4">
            <p>
              Registra que o valor de{' '}
              <strong className="text-cream">{formatBRL(order.totalCents)}</strong> foi devolvido ao
              cliente. O estorno em si deve ser feito no provedor de pagamento; aqui o pedido passa
              para "Reembolsado" e o estoque é liberado.
            </p>
            <Textarea
              label="Motivo"
              name="refundReason"
          ...
          <tbody>
            {order.items.map((item) => (
              <tr key={item.id}>
                  ...
                  <div className="flex items-center gap-3">
                    {item.imageUrl ? (
                      <img
                        src={item.imageUrl}
                        alt=""
                        className="h-12 w-12 shrink-0 rounded-sm bg-noir object-cover"
                        loading="lazy"
                      />
                    ) : (
                      <span
                        className="flex h-12 w-12 shrink-0 items-center justify-center rounded-sm bg-noir text-muted"
                        aria-hidden="true"
                      >
                    ...
                    <div className="min-w-0">
                      {item.productId ? (
                        <Link
                          ...
                          className="block font-medium text-cream hover:text-gold"
                        >
                          {item.name}
                        </Link>
                      ) : (
                      ...
                      <span className="block text-xs text-muted">
                        {item.variantName} · {t('catalog.sku')}{' '}
                      ...
                      {item.productSlug && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="mt-0.5 inline-flex items-center gap-1 text-[10px] uppercase tracking-[0.15em] text-muted hover:text-gold"
                        >
                ...
                <td className="whitespace-nowrap text-right text-ivory/80">
          ...
          {events.map((event) => {
            ...
            return (
              <li key={event.id} className="relative">
                <span
                  className={`absolute -left-[26px] top-1.5 h-2.5 w-2.5 rounded-full ring-4 ring-dark ${DOT_CLASS[tone]}`}
                  aria-hidden="true"
                />
                <div className="flex flex-wrap items-center gap-2">
...
/** Par rótulo/valor; `stack` mantém o valor abaixo do rótulo (para e-mails e IDs longos). */
...
function Money({
  ...
  return (
    <div className="flex items-baseline justify-between gap-4">
      ...
      <dd className={`whitespace-nowrap font-medium ${className}`}>
        {cents < 0 ? `− ${formatBRL(-cents)}` : formatBRL(cents)}
      </dd>
    </div>
  );
}
