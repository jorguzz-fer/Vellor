...
import { type AdminProduct, PRODUCT_STATUS_LABELS, PRODUCT_STATUSES, type ProductStatus } from '@vellor/shared';
...
function totalStock(product: AdminProduct): { stock: number; reserved: number } {
  return product.variants.reduce(
    (acc, variant) => ({ stock: acc.stock + variant.stockQuantity, reserved: acc.reserved + variant.reservedQuantity }),
    { stock: 0, reserved: 0 },
  );
}
...
export default function ProductsPage() {
  ...
  const categories = useQuery({ queryKey: ['admin', 'categories'], queryFn: adminApi.categories, staleTime: 5 * 60_000 });
  ...
  return (
    <div className="mx-auto max-w-7xl">
      ...
      <form onSubmit={submitSearch} className="card mb-6 grid gap-3 p-4 md:grid-cols-[1fr_180px_220px_auto] md:items-end" role="search" aria-label="Filtrar produtos">
        <div className="relative">
          <Input
            ...
            autoComplete="off"
          />
          <Search className="pointer-events-none absolute bottom-3 right-3 h-4 w-4 text-muted" aria-hidden="true" />
        </div>
        <Select
          label="Status"
          name="status"
        ...
        <div className="flex flex-wrap items-center gap-3 pb-1 md:pb-2.5">
          <Checkbox
            ...
            label="Estoque baixo"
            checked={lowStock}
            onChange={(event) => updateParams({ lowStock: event.target.checked ? 'true' : undefined })}
          />
          <Button type="submit" variant="secondary" size="sm">
            Filtrar
          </Button>
          {hasFilters && (
            <button type="button" className="btn-ghost px-2 py-2 text-[10px]" onClick={() => setSearchParams(new URLSearchParams())}>
      ...
      {products.isLoading ? (
        ...
        <ErrorState message={products.error instanceof Error ? products.error.message : undefined} onRetry={() => products.refetch()} />
      ) : !products.data || products.data.items.length === 0 ? (
        <div className="card">
          <EmptyState
            icon={<Package className="h-8 w-8" />}
            title={hasFilters ? 'Nenhum produto encontrado' : 'Nenhum produto cadastrado'}
            text={hasFilters ? 'Ajuste os filtros ou limpe a busca para ver todos os produtos.' : 'Cadastre a primeira peça da curadoria para começar a vender.'}
            action={
              hasFilters ? (
                <Button type="button" variant="secondary" onClick={() => setSearchParams(new URLSearchParams())}>
                  Limpar filtros
                </Button>
              ) : (
        ...
        <>
          <div className={`card overflow-x-auto transition-opacity ${products.isFetching ? 'opacity-60' : ''}`} aria-busy={products.isFetching}>
                    ...
                    >
                      <td>
                        <div className="relative h-12 w-12 overflow-hidden rounded-sm border border-line">
                          <ProductImage src={product.primaryImageUrl} alt={product.name} kind={product.categoryKind} className="absolute inset-0" />
                        </div>
                      </td>
                      <td>
                        <Link
                      ...
                      <td>
                        ...
                        {product.collectionName && <div className="text-xs text-muted">{product.collectionName}</div>}
                      </td>
                      ...
                      <td className="text-right">
                        <span className={`font-semibold ${low ? 'text-danger' : 'text-cream'}`}>{stock}</span>
                        {low && (
                          <span className="ml-1.5 align-middle">
                            <Badge tone="danger">Baixo</Badge>
                          </span>
                        )}
                        {reserved > 0 && <div className="text-[11px] text-muted">{reserved} reservada(s)</div>}
                      </td>
                      <td>
                        <Badge tone={STATUS_TONE[product.status]}>{PRODUCT_STATUS_LABELS[product.status]}</Badge>
                      </td>
                      <td className="whitespace-nowrap text-xs text-muted">{formatDateTime(product.updatedAt)}</td>
                    </tr>
                  );
                })}
              </tbody>
