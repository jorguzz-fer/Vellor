...
export default function ContactsPage() {
  ...
  const params = {
    ...
    pageSize: PAGE_SIZE,
    status:
      statusParam && (CONTACT_STATUSES as readonly string[]).includes(statusParam)
      ...
      {query.isPending ? (
        ...
        <div className="card">
          <EmptyState
            ...
            text={
              params.status
                ? `Não há mensagens com status "${CONTACT_STATUS_LABELS[params.status]}".`
                : 'As mensagens enviadas pelo formulário de atendimento aparecerão aqui.'
            }
          />
        </div>
      ) : (
              ...
              <tbody>
                {data.items.map((contact) => (
                  ...
                  >
                    ...
                    <td>
                      <Link
                        to={`/admin/atendimento/${contact.id}`}
                        className={`hover:underline ${contact.status === 'new' ? 'font-semibold text-cream' : 'font-medium text-gold'}`}
                      >
                        {contact.name}
                      </Link>
                      {contact.productName && (
