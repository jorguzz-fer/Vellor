...
const VALUES: Array<{ icon: ComponentType<{ className?: string }>; title: string; text: string }> =
  [
    {
      icon: ShieldCheck,
      title: t('home.valueAuthenticity'),
      text: t('home.valueAuthenticityText'),
    },
...
export default function HomePage() {
  ...
  return (
    <div className="flex flex-col">
      ...
      <section
        className="mx-auto w-full max-w-7xl px-4 py-14 md:px-8 md:py-20"
...
function Hero() {
  return (
    <section className="relative overflow-hidden border-b border-line bg-noir">
      <div
        className="absolute inset-0 bg-linear-to-b from-noir via-deep to-deep"
        aria-hidden="true"
      />
      <div
        className="absolute -left-40 top-1/3 h-[28rem] w-[28rem] rounded-full bg-gold/10 blur-3xl"
        aria-hidden="true"
      />
      <div
        className="absolute -right-24 -top-32 h-96 w-96 rounded-full bg-spruce blur-3xl"
        aria-hidden="true"
      />
      <div
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-20 md:px-8 md:py-28 lg:py-36">
        ...
        <h1 className="heading mt-5 max-w-3xl text-4xl leading-[1.05] md:text-6xl">
          {t('home.heroTitle')}
        </h1>
        <p className="mt-6 max-w-xl text-sm leading-relaxed text-ivory/80 md:text-base">
          {t('home.heroText')}
        </p>
      ...
      <ul className="mx-auto grid max-w-7xl grid-cols-1 divide-y divide-line sm:grid-cols-2 sm:divide-y-0 lg:grid-cols-4 lg:divide-x">
        {VALUES.map(({ icon: Icon, title, text }) => (
          <li key={title} className="flex gap-4 px-4 py-6 md:px-8">
            <span
              className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-gold/40 text-gold"
              aria-hidden="true"
            >
              <Icon className="h-4 w-4" />
            </span>
            <div>
              <h2 className="font-sans text-[11px] font-bold uppercase tracking-[0.2em] text-cream">
                {title}
              </h2>
...
function SeeAllLink({ to }: { to: string }) {
  return (
    <Link
      to={to}
      className="inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-gold hover:text-gold-light"
    >
...
function ProductsBlock({
  ...
  if (query.data.items.length === 0) {
    return (
      <p className="rounded-sm border border-dashed border-line px-4 py-8 text-center text-sm text-muted">
        Em breve novas peças nesta seleção.
      </p>
    );
  }
  ...
  return (
    <section className="border-t border-line bg-dark/40" aria-labelledby={id}>
      <div className="mx-auto grid max-w-7xl gap-6 px-4 py-14 md:px-8 md:py-20 lg:grid-cols-12">
        <div
          className={`relative flex min-h-64 flex-col justify-end overflow-hidden rounded-sm border border-line bg-noir p-8 lg:col-span-4 ${reverse ? 'lg:order-2' : ''}`}
        >
          <div
          ...
          />
          <div
            className="pointer-events-none absolute -right-10 top-6 h-40 w-40 rounded-full border border-gold/15"
            aria-hidden="true"
          />
