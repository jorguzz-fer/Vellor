...
import { PRODUCT_SORT_LABELS, PRODUCT_SORTS, type ProductQuery, type ProductSort } from '@vellor/shared';
...
const SORT_OPTIONS = PRODUCT_SORTS.map((sort) => ({ value: sort, label: PRODUCT_SORT_LABELS[sort] }));

function parseSort(value: string | null): ProductSort {
  return value !== null && (PRODUCT_SORTS as readonly string[]).includes(value) ? (value as ProductSort) : 'featured';
}
...
export default function CategoryPage({ slug, mode }: { slug?: string; mode?: 'search' }) {
  ...
  const title = isSearch
    ...
    : (category?.name ?? (categorySlug ? (KNOWN_CATEGORY_NAMES[categorySlug] ?? humanizeSlug(categorySlug)) : t('catalog.allProducts')));
  ...
  const updateParams = (patch: Record<string, string | null | undefined>, options: { keepPage?: boolean } = {}) => {
  ...
  const countLabel = total === undefined ? null : total === 1 ? t('catalog.productCount', { count: 1 }) : t('catalog.productsCount', { count: total });
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 md:px-8">
      ...
      <header className="relative mb-8 overflow-hidden rounded-sm border border-line bg-dark">
        {category?.heroImageUrl && (
          <img src={category.heroImageUrl} alt="" className="absolute inset-0 h-full w-full object-cover opacity-40" loading="eager" />
        )}
        <div className="absolute inset-0 bg-linear-to-r from-noir via-noir/80 to-transparent" aria-hidden="true" />
        <div className="relative px-6 py-10 md:px-10 md:py-14">
          <nav aria-label="Navegação estrutural" className="mb-4 flex items-center gap-1.5 text-[11px] uppercase tracking-[0.15em] text-muted">
          ...
          {!isSearch && category?.description && <p className="mt-3 max-w-2xl text-sm leading-relaxed text-ivory/75">{category.description}</p>}
          {isSearch && (
            <form onSubmit={submitSearch} role="search" className="mt-5 flex max-w-xl flex-col gap-2 sm:flex-row">
              <Input
                name="q"
      ...
      {!isSearch && collections.data && collections.data.length > 0 && (
        <div className="mb-6">
          ...
          <div className="-mx-4 flex gap-2 overflow-x-auto px-4 pb-1 scrollbar-none md:mx-0 md:flex-wrap md:px-0" role="group" aria-label={t('catalog.collections')}>
            <Chip active={!collectionSlug} onClick={() => updateParams({ colecao: null })}>
              {t('common.all')}
            </Chip>
            {collections.data.map((c) => (
              <Chip key={c.id} active={collectionSlug === c.slug} onClick={() => updateParams({ colecao: c.slug })}>
                {c.name}
                {c.productCount !== undefined && <span className={`ml-1.5 ${collectionSlug === c.slug ? 'text-noir/70' : 'text-muted'}`}>{c.productCount}</span>}
              </Chip>
            ))}
          </div>
        </div>
      ...
      <div className="mb-6 flex flex-col gap-4 border-y border-line py-4 lg:flex-row lg:items-end lg:justify-between">
        <form onSubmit={applyPrice} className="flex flex-wrap items-end gap-2">
          <fieldset className="flex flex-wrap items-end gap-2">
            ...
            <Input
              ...
              wrapperClassName="w-28"
            />
            <Button type="submit" variant="secondary" icon={<SlidersHorizontal className="h-3.5 w-3.5" />}>
              {t('common.filter')}
            </Button>
          </fieldset>
          {hasFilters && (
            <Button type="button" variant="ghost" onClick={clearFilters} icon={<X className="h-3.5 w-3.5" />}>
        ...
        <div className="flex items-end justify-between gap-4 lg:justify-end">
          ...
          <Select
            ...
            value={sort}
            onChange={(e) => updateParams({ ordem: e.target.value === 'featured' ? null : e.target.value })}
      ...
      {!enabled ? (
        ...
        <EmptyState
          ...
          text={isSearch ? 'Tente outro termo ou explore as categorias principais.' : 'Ajuste a faixa de preço ou a coleção para ver mais peças.'}
          action={
            hasFilters ? (
              <Button variant="secondary" onClick={clearFilters} icon={<X className="h-3.5 w-3.5" />}>
                {t('common.clearFilters')}
              </Button>
            ) : (
              <div className="flex flex-col gap-3 sm:flex-row">
        ...
        <>
          ...
          <Pagination page={products.data.page} totalPages={products.data.totalPages} onChange={goToPage} />
        </>
      )}
    </div>
  );
  ...
  return (
    <button
      ...
      className={`shrink-0 rounded-full border px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.15em] transition-colors ${
        active ? 'border-gold bg-gold text-noir' : 'border-line-strong text-ivory/80 hover:border-gold hover:text-gold'
      }`}
    >
      {children}
    </button>
