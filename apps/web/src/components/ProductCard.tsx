...
export function ProductCard({ product, onQuickAdd }: ProductCardProps) {
  ...
  const summaryAttributes = attributesForKind(product.categoryKind)
    .filter(
      (f) =>
        f.summary && product.attributes[f.key] !== undefined && product.attributes[f.key] !== '',
    )
    .slice(0, 3);
  ...
  return (
    <article
      className="card group relative flex flex-col overflow-hidden transition-colors hover:border-gold/50"
      ...
      <div className="flex flex-1 flex-col gap-1.5 border-t border-line/70 p-4">
        ...
        {summaryAttributes.length > 0 && (
          <p className="line-clamp-1 text-xs text-muted">
            {summaryAttributes
              .map((f) => formatAttributeValue(f, product.attributes[f.key]!))
              .join(' · ')}
          </p>
        )}
...
export function ProductGrid({
  ...
  columns?: 2 | 3 | 4;
  onQuickAdd?: (p: ProductSummary) => void;
}) {
  const cols =
    columns === 2
      ? 'sm:grid-cols-2'
